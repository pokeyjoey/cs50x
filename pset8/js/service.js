...
// global reference to placemarks
var placemarks = [];

// global reference to markers
var markers = [];
...
function pickup()
{
    // if passenger is within 15 meters of bus, and at least one seat on the
    // bus is empty:
    // - give the passenger a seat on the shuttle
    // - remove passenger from 2D map
    // - remove passenger from 3D earth.
    // else if no passengers within 15 meters of bus
    // - announce nobody to pick up

    // calculate shuttle distance from passengers
    //var distance = shuttle.distance(lat, lng);

    // remove a placemark from 3D Earth
    //var features = earth.geFeatures();
    //features.removeChild(placemark);

    // remove marker form 2D map
    //marker.setMap(null);
...
function populate()
{
    ...
    for (var i = 0; i < PASSENGERS.length; i++)
    {
        ...
        placemarks[PASSENGERS[i].name] = placemark;
        ...
        markers[PASSENGERS[i].name] = marker;
        console.log(markers);
